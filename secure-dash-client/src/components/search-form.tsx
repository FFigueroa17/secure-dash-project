...
export function SearchForm() {
  return (
    <SidebarGroup className="py-0 pt-3">
...
export function SearchCommandMenu() {
  ...
  return (
    <>
      <Button
        className="flex justify-between text-muted-foreground items-center gap-2 w-full h-10 bg-transparent"
        variant="outline"
      ...
      <CommandMenu.Dialog open={open} onOpenChange={setOpen}>
        ...
        <div className="group/cmd-input bg-bg-white-0 flex h-12 w-full items-center gap-2 px-5">
          ...
          />
          <CommandMenu.Input placeholder="Buscar o navegar a" />
          <Kbd.Root>
            {/* ⌘K */}
            <svg
              width="19"
        ...
        <div className="px-4 py-4">
          <div className="text-xs text-text-sub-600 mb-3">Buscando en</div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary">Dashboard</Badge>
            <Badge variant="secondary">IPs Bloqueadas</Badge>
            <Badge variant="secondary">Logs de Fail2Ban</Badge>
        ...
        <CommandMenu.List>
          <CommandMenu.Group>
            <CommandMenu.Item onSelect={() => handleNavigate('/app/dashboard')}>
              <LayoutDashboard className="size-4" />
              Dashboard de seguridad
            </CommandMenu.Item>
            <CommandMenu.Item
              onSelect={() => handleNavigate('/app/banned-ips')}
            >
              <Shield className="size-4" />
              Lista de IPs bloqueadas
            </CommandMenu.Item>
            <CommandMenu.Item onSelect={() => handleNavigate('/app/')}>
              <ChartNetwork className="size-4" />
              Logs de Fail2Ban
            </CommandMenu.Item>
            <CommandMenu.Item onSelect={() => handleNavigate('/app/users')}>
        ...
        <CommandMenu.Footer>
          <div className="flex gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-text-sub-600">Navegar</span>
            </div>
            <div className="flex items-center gap-2">
              <CommandMenu.FooterKeyBox>
                <CornerDownLeft className="size-3" />
              </CommandMenu.FooterKeyBox>
              <span className="text-xs text-text-sub-600">Seleccionar</span>
            </div>
          </div>
        </CommandMenu.Footer>
      </CommandMenu.Dialog>
